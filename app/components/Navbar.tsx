...
function Navbar() {
  ...
  try {
    ...
    // Not in provider context, use default
  }

  // Translate navbar items
  const homeText = t("Home");
  const careerPathText = t("Career Path");
  const dmitText = t("DMIT");
  const psychometricText = t("Psychometric test");
  const resourcesText = t("Resources");
  const blogText = t("Blog");
  const contactText = t("Contact");
  const bookConsultationText = t("Book Consultation");
